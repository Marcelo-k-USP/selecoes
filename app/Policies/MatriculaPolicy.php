...
class MatriculaPolicy
{
    ...
    public function view(User $user, Matricula $matricula)
    {
        ...
        elseif (Gate::allows('perfilgerente'))
            return $user->gerenciaPrograma($matricula->selecao->programa_id, $matricula->selecao->vinculo_id);
        elseif (Gate::allows('perfildocente'))
            return $user->gerenciaProgramaGrupoFuncao('Docentes do Programa', $matricula->selecao->programa_id, $matricula->selecao->vinculo_id);
        else
    ...
    public function create(User $user, ?Selecao $selecao = null)
    {
        if (!is_null($selecao)) {
            ...
            if (!(str_starts_with($selecao->estado, 'Período de') && str_contains($selecao->estado, 'Matrículas')))
                return false;
        }
